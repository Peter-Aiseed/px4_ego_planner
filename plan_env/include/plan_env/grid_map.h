...
struct MappingParameters {
  ...
  /* Fixed global voxel coordinate origin */
...
inline bool GridMap::isInMap(const Eigen::Vector3i& idx) {
  ...
  for (int i = 0; i < 3; ++i)
  {
    ...
    min_id(i) = mp_.current_center_id_(i) - half_low;
    max_id(i) = mp_.current_center_id_(i) + half_high;
  }

  return (idx(0) >= min_id(0) && idx(0) <= max_id(0)) &&
