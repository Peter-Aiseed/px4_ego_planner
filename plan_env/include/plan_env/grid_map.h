...
struct MappingParameters {
  ...
  /* Ring Buffer */
  Eigen::Vector3i local_map_size_id_;  // Grid dimensions of the rolling window (e.g. 100x100x50)
  Eigen::Vector3i current_center_id_;  // Voxel ID of the drone's current position in world space
...
/* Ring Buffer Help Function*/
// ADD THIS METHOD inside grid_map.h:
...
inline int GridMap::toAddress(const Eigen::Vector3i& id) {
  return toBufferAddress(id);
}

inline int GridMap::toAddress(int& x, int& y, int& z) {
  return toBufferAddress(Eigen::Vector3i(x, y, z));
}
...
inline int GridMap::getOccupancy(Eigen::Vector3i id) {
  if (!isInMap(id)) return -1; // Outside local window is treated as unobserved
  return md_.occupancy_buffer_[toAddress(id)] > mp_.min_occupancy_log_ ? 1 : 0;
}

// 1. World Position (Vector3d) relative check
inline bool GridMap::isInMap(const Eigen::Vector3d& pos) {
  Eigen::Vector3i idx;
  posToIndex(pos, idx); // Convert position to index first
  return isInMap(idx);  // Pass to the index-based check below
}

// 2. Grid Index (Vector3i) relative check
